...
## 2. Information Architecture: Two-Level Experience
...
└──────────────────────────────┬──────────────────────────────┘
                               │
                [ SPIDER-WEB SHOOTING TRANSITION ]
                               │
                               ▼
┌─────────────────────────────────────────────────────────────┐
...
│ - Core Engineering Responsibilities (Frontend/Backend/RBAC) │
│ - Verified Tenure Timeline (May 2026 → June 2026)           │
│ - Cross-domain semantic links (Skills, About, Projects)     │
└─────────────────────────────────────────────────────────────┘
```

---

## 3. Engineering Architecture Flow
1. **Tier 01 // Frontend Client**: `React + TypeScript` (modular component tree, type-safe models).
2. **Tier 02 // Interface Layer**: `REST APIs` (structured endpoint contracts and payload validation).
3. **Tier 03 // Backend Engine**: `ASP.NET Core & C#` (async service controllers and business domain logic).
4. **Tier 04 // Security Boundary**: `RBAC Authorization` (strict role verification and policy enforcement).

---

## 4. Navigation & Cross-Section Connections
- **Skills Taxonomy**: Interactive architecture cards link directly to corresponding technical nodes (`ASP.NET Core`, `React & Vite`, `TypeScript`) in the Skills web.
